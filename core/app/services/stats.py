...
from app.utils.logger import get_logger, log_metadata

logger = get_logger("app.services.stats")


def get_wrap(chats: str) -> dict:
    logger.info("Starting wrap generation for year %s", YEAR)
    try:
        ...
        logger.info("Wrap generation completed successfully")
        return stats
    except Exception as exc:
        logger.error("Failed to generate wrap: %s", exc, exc_info=True)
        raise
